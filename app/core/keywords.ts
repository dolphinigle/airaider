// KEYWORD POOLS (GENERATION_FLOW.md §5) — the engine's story-seed vocabulary.
// Sampling: 1 BOND + 1 TIE + 1–2 WILDCARDS (wildcards = union of THINGS/OCCASIONS/PEOPLE/UNCANNY/MOODS).
// STYLE BAR: bare words ("wedding" not "a wedding"; article only where the bare word changes meaning);
// 1–3 words; evocative-generic, never a micro-premise; low-medieval; no proper nouns; no cross-pool dupes.
// Append freely — these lists are MEANT to grow.
...
// ---- the sampler (GENERATION_FLOW.md §5) -------------------------------------------------------
const WILDCARDS: string[] = [...THINGS, ...OCCASIONS, ...PEOPLE, ...UNCANNY, ...MOODS];
const kpick = (a: string[], r: () => number) => a[Math.floor(r() * a.length)];

/** Draw the story keywords: 1 BOND + 1 TIE + 1–2 wildcards (2 at 75% — variety without formula). */
export function pickKeywords(r: () => number): string {
  const out = [kpick(BOND, r), kpick(TIE, r), kpick(WILDCARDS, r)];
  if (r() < 0.75) {
    let w = kpick(WILDCARDS, r);
    if (w === out[2]) w = kpick(WILDCARDS, r);
    out.push(w);
  }
  return out.join(', ');
}
