...
export class OpenAINarrator implements Narrator {
  ...
  async cardAsk(i: CardAskInput): Promise<CardAskOut> {
    const system =
      ...
      `  "offeredReward": { "kind": "what the company GETS — one of: gold | captive (a prisoner to ransom/hold) | recruit (a fighter who joins you) | item | unknown (a MYSTERY — a sealed shrine, an unexplored ruin; the spoils aren't known up front) | none (a desperate plea, no pay). Pick what FITS the job + JOB TYPE (a raid pays gold; a hunt/capture yields a captive; a rescue can win a recruit; an unexplored place is unknown).", "label": "PLAYER-FACING <=8 words naming it plainly: 'a fat purse' / 'the captured smuggler, to ransom' / 'whatever the shrine holds' / 'nothing — they're begging'. NEVER a number." },\n` +
