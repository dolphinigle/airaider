...
export class OpenAINarrator implements Narrator {
  ...
  async chainBeat(i: ChainBeatInput): Promise<ChainBeatOut> {
    const system =
      ...
      `  "offeredReward": { "kind": "one of: gold | captive | recruit | item | unknown | none — what the company stands to GET. The kind MUST name the HEADLINE of your label: a purse/coin/pay = gold; a person to hold = captive; a person who joins = recruit; a thing = item; 'unknown' ONLY when the spoils genuinely can't be known yet.", "label": "PLAYER-FACING <=8 words naming it plainly, in the SPEAKER'S OWN TERMS (what this client/scene actually puts on the table). NEVER a number." },\n` +
