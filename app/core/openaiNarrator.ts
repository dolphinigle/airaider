...
export class OpenAINarrator implements Narrator {
  ...
  async cardAsk(i: CardAskInput): Promise<CardAskOut> {
    const system =
      ...
      `  "offeredReward": { "kind": "the engine has ALREADY rolled the reward — its kind is given below as REWARD. Set kind to that SAME value, EXCEPT use 'unknown' if this is a genuine mystery job (a sealed shrine/unexplored ruin where the player shouldn't know the spoils yet).", "label": "PLAYER-FACING <=8 words naming the reward plainly: 'a fat purse' / 'the captured smuggler, to ransom' / 'whatever the shrine holds'. NEVER a number." },\n` +
