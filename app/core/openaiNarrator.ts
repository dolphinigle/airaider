...
export class OpenAINarrator implements Narrator {
  ...
  async cardAsk(i: CardAskInput): Promise<CardAskOut> {
    const system =
      ...
      `  "offeredReward": { "kind": "the engine has ALREADY rolled the reward — its kind is given below as REWARD. Set kind to that SAME value, EXCEPT use 'unknown' if this is a genuine mystery job (a sealed shrine/unexplored ruin where the player shouldn't know the spoils yet).", "label": "PLAYER-FACING <=8 words naming the reward plainly. For coin, VARY the phrasing job-to-job (a fat purse · coin up front · a merchant's cut · a reaper's fee) — do NOT always write 'a heavy purse of gold'. For a unit: 'the captured smuggler, to ransom' / 'a sworn recruit'. NEVER a number." },\n` +
  ...
  async genesis(i: GenesisInput): Promise<GenesisOut> {
    ...
    const system =
      ...
      `${twistBlock}\n` +
      `PLAN THE ARC — output "arc": a ROUGH ordered guide of about ${eb} steps (a skeleton, NOT a rigid script; ~${eb} because this chain's length/stakes warrant it). EACH STEP — the first included — is ONE COMPLETE, QUEST-WORTHY MISSION the player sends mercs on: a self-contained job that can succeed or fail and comes away with a result, never a fragment or a vague phase. Merely meeting or accepting is NOT a mission (the player takes the job by choosing it); a mission spans setting out, the deed, and the result the way 'rob the bank' spans the approach, the break-in, and the getaway. Middle steps escalate the obstacle; the last step is the finale that brings it to a head. Do NOT resolve the chain before the last step. Each step is a short phrase.\n` +
    ...
    const coreFate = i.coreKind === 'captive'
      ? ' The company\'s end goal is to TAKE THEM — a quarry the saga hunts, corners, and holds; let that pursuit shape who they are and why they\'re worth the chase.'
      : i.coreKind === 'recruit'
        ? ' The company\'s end goal is to WIN THEM — someone whose loyalty the saga earns and who joins by the end; let that shape who they are and why they\'re worth keeping.'
        : '';
    const core = i.personal
      ...
      : `CORE PERSON tags: [${i.focalTags[0]?.join(', ')}]. Invent and NAME them; the story centers on them.${coreFate}`;
  ...
  async chainBeat(i: ChainBeatInput): Promise<ChainBeatOut> {
    const system =
      ...
      `  "offeredReward": { "kind": "one of: gold | captive | recruit | item | unknown (a mystery — spoils not known yet) | none — what the company stands to GET (coin for the step, and/or the saga's prize)", "label": "PLAYER-FACING <=8 words, plain, NEVER a number — e.g. 'a fat purse' / 'the smith, to recruit' / 'whatever lies within' / 'little but goodwill'" },\n` +
