...
export class OpenAINarrator implements Narrator {
  ...
  async cardAsk(i: CardAskInput): Promise<CardAskOut> {
    const system =
      ...
      `Output JSON only:\n` +
      `{ "situation": "the player's ONLY text for this job (2-3 plain sentences). Open from the 'how it reaches you' line below, WOVEN into real sentences — NEVER quote or echo that instruction. Whoever brings it MUST speak ONE line of DIALOGUE that states the WORK plainly and the DRAW (coin, a person to save or take, a threat to remove) — but NEVER a number or amount: say 'good coin' / 'a fat purse', NEVER '40 gold'. The job MUST match the JOB TYPE given below (see meanings) and be SPECIFIC to this place/people. Invent it FRESH — do NOT reuse any example's words or the same job each time.",\n` +
  ...
  async chainBeat(i: ChainBeatInput): Promise<ChainBeatOut> {
    const system =
      ...
      `Output JSON only:\n` +
      `{ "situation": "the player's ONLY text for this job (2-4 plain sentences). The player is the BOSS at their fort deciding whether to send mercs on this step — so write WHAT REACHES THEM THIS TURN: the petitioner/job arriving (the opener), OR one of your own mercs / a runner bringing word of the next step, OR a development pressing in. Use a line of DIALOGUE where someone speaks. Make plain what the company would DO if sent, and what's at stake. Do NOT narrate the company already out in the field doing it (that happens AFTER you send them). READABILITY: clean plain sentences read once and understood — NOT fragment-stacks ('Grey morning. Mud. A man.'), NOT run-ons, NO flowery diction ('blisters the reedsea'), NO invented/obscure coinages — name things plainly. ORIENT a person ONCE on first appearance (natural apposition, not parens); a name in the ALREADY-MET list uses their BARE name.",\n` +
