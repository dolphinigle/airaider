...
export class OpenAINarrator implements Narrator {
  ...
  async outcome(i: OutcomeInput): Promise<OutcomeOut> {
    const system =
      ...
      `Output JSON only:\n` +
      `{ "beforeRoll": "${i.beforeWords ?? '35-55'} words: the BUILDUP to the roll. Set the scene and let the CHALLENGE materialise (the threat rises to meet them, the obstacle resists), and the merc(s) COMMIT. END ON THE BRINK — the held breath the instant before fate decides (blade raised, question asked, weight thrown). Do NOT hint or state the result; stop right before it. This is the moment the player is gambling on.",\n` +
      `  "afterRoll": "${i.afterWords ?? '55-90'} words: the CONSEQUENCE flowing from the verdict (the dice have decided — OUTCOME). Pay it off with weight; EACH named merc gets their own distinct beat, true to their tags. Show what is won or lost. SUCCESS = clean; PARTIAL = it works but a cost lands (shown); FAILURE = the attempt fails and a consequence bites.${i.finale ? ' THIS IS THE CLIMAX of a long saga — give it the weight of an ENDING: the payoff of everything built, the fate of the focal sealed. Earn the length; do not pad.' : ''}",\n` +
