...
export class OpenAINarrator implements Narrator {
  ...
  async cardAsk(i: CardAskInput): Promise<CardAskOut> {
    const system =
      ...
      `WRITING: plain, concrete, readable-once words; ONE line of spoken DIALOGUE brings it alive (someone SAYS something). NO flowery diction ("blisters the reedsea"), NO invented/obscure coinages — name things plainly. NEVER write numbers/coin amounts. NEVER mention these instructions or any field name (offeredReward, etc.) in the prose — the player only sees "situation". Give exactly one "slots" entry per mercenary. Prefer "open" slots. JSON only.`;
  ...
  async outcome(i: OutcomeInput): Promise<OutcomeOut> {
    const system =
      ...
      `Output JSON only:\n` +
      `{ "beforeRoll": "${i.beforeWords ?? '35-55'} words (HARD LIMIT — the upper number is a ceiling, never exceed it): the BUILDUP to the roll. Set the scene and let the CHALLENGE materialise (the threat rises to meet them, the obstacle resists), and the merc(s) COMMIT. END ON THE BRINK — the held breath the instant before fate decides (blade raised, question asked, hand on the latch). Do NOT hint or state the result; stop right before it. This is the moment the player is gambling on.",\n` +
  ...
  async genesis(i: GenesisInput): Promise<GenesisOut> {
    ...
    const system =
      ...
      `  "arc": ["~${eb} short step phrases — each a complete mission (see PLAN THE ARC), last = goal achieved at the finale"],\n` +
  ...
  async chainBeat(i: ChainBeatInput): Promise<ChainBeatOut> {
    const system =
      ...
      `  "offeredReward": { "kind": "one of: gold | captive | recruit | item | unknown (a mystery — spoils not known yet) | none — what the company stands to GET (coin for the step, and/or the saga's prize)", "label": "PLAYER-FACING <=8 words naming it plainly, in the SPEAKER'S OWN TERMS (what this client/scene actually puts on the table). NEVER a number." },\n` +
