...
- **archetype** — the kind of job (raid / capture / rescue / escort / investigate / hunt / contract…), gated by fort capability (every captive-payer needs a Dungeon; lead-hunt needs a Scout). Does triple duty: info for the player's pursue decision, a seed for AI generation, and the bias for the reward kind.
  - 🛠 2026-08-28 (designer: *"expand the list of one off quest types"* / *"generate 100 to make it not monotonous"*): the pool is a DATA TABLE of ~110 rows in `src/engine/archetypes.ts` — each row a gloss, a reward **profile**, a slot range and an optional fort gate. Only the drawn row's gloss is ever rendered, so the pool costs the prompt eight archetypes did. Rewards switch on the eight profiles (`spoils · captive · recruit · relic · lead · coin · find · bloody`), never on the name, which is why the list can grow without touching the economy. The archetype is the strongest single input-shaping lever on a one-off card: it is the dealt fact that decides what the job IS.
  - The player is shown the **gloss**, not the slug, wherever the lead has no title yet: with eight archetypes the slug was learnable, with a hundred and nine it is a filing code.
  - A row's NAME outranks its gloss when the two disagree — `field-picking` wrote herb-gathering and `clearance` wrote a boasting hunter, against glosses that said otherwise. Name rows for what the job IS; a gloss cannot rescue a misleading name.
...
- **Chain = reward, storied.** At genesis the engine **generates the chain's FOCAL CHARACTER first** — the person the saga is about — at the saga's payoff value, plus a **likely fate** (recruit / captive / gold-hoard — §2's extendable kind table). The AI then authors the bible **around that character** (the "baseline guy"; the AI may flesh them out further; *their tags are the story seed* — a rolled `bg:princess` hands the AI "a crown in exile"). For a **main chain** (merc-join), the focal character is the **existing merc** — the chain develops *them* (pinned CORE memories, a meaningful event — tag-stamping is post-prototype) rather than acquiring someone new. The focal character can be **new or known** (a rare chain's payoff might be re-engaging a long-lost veteran, or capturing a recurring antagonist — recurrence and attachment become one engine).

This makes the payoff a person you've spent the whole arc with — you *know* them before you get them. The **vague direction** ("likely ends with a powerful recruit") gives the AI a climax to write toward *and* the player a visible long-horizon goal. But the **fate is play-determined**: the engine sets the *likely* outcome, the finale **roll** decides the *actual* one (success → they join clean; partial → they join lesser — saddled with a debt/liability; failure → they **slip away for now**: the bank is forfeit and they exit to the lore graph — alive, findable — with a rarity-priced **sequel lead** as the road back; the AI may instead deliver them in a worst form — bitter captive, debt-saddled — when the fiction supports it. §21-4a). The gamble is the SEASON — a saga's worth of banked merc-days — never the person (permanent loss ≈ never; out of prototype scope). Every chain has a focal CHARACTER (§2 lock — even a gold-hoard saga is about a person, e.g. the hoard's guardian); the *prize kind* varies, the person never disappears.

One-offs are **progression fuel** — the steady supply of tagged captives + gold that feeds the fort — not narrative. Their pipeline: (1) engine generates the **reward at quest-birth** (kind + value + tags, fixed then) + a templated card + ask (zero AI for common; richer for rare); player sees the reward **envelope**; (2) assign → roll → **success / partial / failure**; (3) the outcome **applies a consequence, never rescaling the unit** (success → reward clean; partial → reward + a **negative-gold liability card** — `evidence` / `a mess` — negative stackables; failure → lose the reward; injuries = the AI-judged channel above, any outcome); (4) **one cheap AI call** writes the outcome line (individuating the assigned merc) *and* describes any acquired captive (name engine-assigned, §4b). Anti-sameness = varied archetype→ask + the loot lottery, not bespoke prose. No bible, no chain.
...
> **2026-07-06 rulings (designer, supersedes the split-card reading):** (a) **`situation` IS the whole card** — the player reads one flowing text; `job` survives only as a terse list/ledger line, never rendered on the card. (b) **Card voice = a BRIEFING to the boss** — second person, present tense, plain declaratives: context → hook → task + hands wanted + qualitative pay/risk (tone calibrated against Fort of Chains quest offers: dispatcher framing, dry-wry on routine work, sober when grave, no dramatized scenes the boss isn't in). (c) **favored/clashing may name personality words** — they contribute via the ±0.5U favored/clash levers only, never via attribute feeds; legal families = skills + personality + the four flavor looks (tall/short/endowed/flat), fenced engine-side (stat body tags would double-dip the attribute feed). (d) **GAME WRITING, not literature** (the governing prose rule): every sentence must give the player something usable — the problem, place, client, task, hands, pay, or risk; mood-only sentences are cut. Think RPG quest text, never novel prose. (e) **Anonymity by omission**: small-gravity one-offs deal NO NPC names — folk stay nameless by trade ("a drover", "the reeve"); serious+ matters get one color name; only the quarry (who becomes a card) always carries a name. A dealt name gravitates a card; an anonymous petitioner keeps a small job small.
...
- **partial** → **half**: keep the unit + a **liability** (a negative stackable, `kind: evidence`/`mess`) sized to net V/2 *(a focal character survives, just saddled with it)*, or give V/2 in gold if not worth keeping (engine threshold — same KEEP≈0.4 rule as the chain bank);
- **failure** → nothing.
...
- **Chain** — the **focal character generated at genesis IS the payoff**, and its value is **accrued over the beats as a merc-day bank** (ECONOMY §5a, [REWARD_BANK.md](REWARD_BANK.md)). Intermediate beats **bank** their merc-cycles toward the focal (each beat also carries a **small engine-set side-loot budget** — §6/§8; a failed beat banks 0 and drops no side-loot); the **finale crystallizes the bank** into the focal character **+ surplus gold**, the focal's **fate decided by the finale roll** (success = clean · partial = lesser/saddled · failure = **bank forfeited + the focal slips away for now** — sequel lead as the road back, §21-4a; permanent loss out of prototype). A bank short of the focal's value delivers them **with a debt**, or — if too thin — lets them **slip away** for salvage gold. A per-chain **failure budget** (harder = fewer) turns repeated stumbles into a forced **last-chance** finale rather than an endless retry.
...
```
 finale card
   ...
   (partial = the lesser/saddled version of that kind · failure = bank forfeit, focal slips away — sequel road back, §21-4a)
```
