...
- 🛠 **2026-07-18 — story-NPC write-back BUILT (engine-side, at saga CLOSE)**: coined bible cast the player actually MET (`introducedNames`) persist as lore-only character nodes, cap 2/saga (client > obstacle > ally), blurb = their cast `who` line, plus one memory edge to the saga's FOCAL (client `party-to`, obstacle `rival-of`, salience 0.5, never core → standard §2 decay forgets them in ~45 cycles unless re-touched). Timing deliberately diverges from a literal §3.3 genesis-time reading: live-chain cast are slate-excluded anyway, and close-time persistence avoids offstage spoilers + abandoned-saga clutter. Recurrence rides the existing channels (slate reuse-before-coining; §21-3 known-cast promotion — which was starved with an empty pool until this). Name integrity: persisted names are checked against ALL nodes incl. inactive, so a remembered name is never re-rolled onto a stranger.
