...
- **Commit-time AI gets only these four fields + party composition** and invents the *entire* premise — including what kind of job this is — from scratch. Maximum creative latitude, lowest possible upfront cost.
...
- **Commit-with-imperfect-info tension** is intentional: you see difficulty + reward magnitude + region, not specific scenarios. **Lead-scouting** (a cunning-hero action) can reveal 1–2 scenario hints on an *existing* lead before commit — a distinct use of social/scout heroes.
