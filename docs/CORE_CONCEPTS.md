...
- **Lead = `{archetype, difficulty_class, reward_budget, region, expiry, hook}`.** Hand-authored archetypes (~15–20: coin_wagon, bandit_camp, lost_heir, haunted_ruin, …). **Leads are 100% engine-generated, zero AI.** Hook is a pure template-string pulled from the archetype's hand-authored hook pool, with `{region}` substitution. No AI call is ever made for a lead. AI cost happens only at *commit*.
- **Hooks are atmospheric, not narrative.** They evoke region, risk, and vibe. They **must not** name NPCs, specific locations beyond region, occupations, hiding places, or any plot specifics. Two leads of the same archetype in the same region should be functionally interchangeable from the hook alone. The actual people, places, and twists are invented *only at commit time*. This is what keeps leads truly cheap — if the hook commits to "Old Murch the tanner owes Lord Kessel," we've already half-written the quest and saved nothing.
