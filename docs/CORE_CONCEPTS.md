...
- **Lead = `{archetype, difficulty_class, reward_budget, region, expiry, hook}`.** Hand-authored archetypes (~15–20: coin_wagon, bandit_camp, lost_heir, haunted_ruin, …). Generated by the engine; hook line is template-pulled from a small per-archetype pool (cheap, often no AI at all).
