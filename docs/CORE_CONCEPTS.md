...
- **Lead = `{archetype, difficulty_class, reward_budget, region, expiry}`.** Hand-authored archetypes (~15–20: coin_wagon, bandit_camp, lost_heir, haunted_ruin, …). **Leads are pure metadata — no hook field, no prose.** 100% engine-generated, zero AI. UI displays the lead as `{Archetype} · {region} · L{dc} · ~{reward}g · {expiry}d`.
- **No hook prose, ever.** Even a templated atmospheric string ("bandits in the forest, local bounty posted") narrows the AI's creative window at commit time — "local bounty posted" forces the generated quest to involve a local bounty. The archetype name alone (`bandit_camp`) carries categorical steering, which is intentional (the player needs to know they're picking a bandit-camp-shape job). Any *additional* prose steering is gratuitous and removed.
- **Commit-time AI gets only:** archetype + difficulty + region + budget + party composition. From these five pieces of metadata the AI invents every concrete detail (NPCs, locations within the region, scenario shapes, twists). Maximum creative latitude, minimum upfront token cost.
...
- **Commit-with-imperfect-info tension** is intentional: you see archetype + difficulty + reward magnitude, not specific scenarios. **Lead-scouting** (a cunning-hero action) can reveal 1–2 scenario hints on an *existing* lead before commit — a distinct use of social/scout heroes.
