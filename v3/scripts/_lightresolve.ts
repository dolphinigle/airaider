...
const light = captured.filter(c => c.input.gravity?.startsWith('a small') && !c.input.chainContext);
...
type Row = { id: string; arm: string; card: string; job: string; outcome: string; delivered: string; before: string; after: string };
const rows: Row[] = [];
for (const c of light) {
  const i = c.input;
  rows.push({ id: '', arm: 'A', card: i.situation ?? '', job: c.job, outcome: i.outcome, delivered: i.deliveredSummary ?? '', before: c.out?.before ?? '', after: c.out?.after ?? '' });
}
