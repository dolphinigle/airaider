  ...
  return {
    ...
    async writeQuest(input: QuestWriteInput): Promise<QuestWriteOut> {
      ...
      const system = [
        ...
        `YOUR OUTPUT — respond as JSON: {title, situation, job, ask: [{attribute, extraAttribute?, favored, clashing, requiredTag?${input.kind === 'beat' ? ', mustBeFocal?' : ''}}]${input.kind === 'finale' ? ', approaches: [{label, rewardKind, attribute, favored}]' : ''}${input.framedCharacter?.partial ? ', quarryTags' : ''}}. Field by field:`,
        '- title: a short, concrete card name — never prefixed with the archetype label.',
        '- situation: THE card the player reads. A common card = 3-5 short sentences, never more, whatever its gravity; uncommon and rare may take up to three short paragraphs. Shape: THE MATTER ITSELF FIRST — what is wrong and where — then who wants it done (how word reached you is worth one clause at most, and when the matter is visible from the walls or already known, no bringer is needed at all) → the task, the hands it wants, the pay and risk — and pay and loot rights ride in their OWN short sentence, never fused with the task or a condition. ONE FACT PER SENTENCE: a sentence carrying a task plus a payment plus a proviso is three sentences. Never open on the arrival of a messenger. Vary what SIGNALS the wrongness — an absence, a sound, animal behavior, damage, a person\'s state — an object left behind is the trade\'s most overused signal, never the default. The task must be unmistakable from this text alone, stated as the outcome wanted — one errand, never a move-by-move itinerary. THE CARD KNOWS ONLY WHAT ITS SOURCES COULD KNOW: what was seen from the walls stays at what eyes can see, a bringer knows what a person plausibly would — a hidden thing may be suspected or rumored, never described as fact (finding out is the job). A client\'s exact words may be quoted when they matter — never as a ritual. Most cards need no forecast line at all; when a risk line appears, give it a fresh shape in flowing prose — never a labeled clause (a leading word and a colon). A briefing states intent, rumor, and risk — never a named person\'s specific future action as a script ("X will meet you and demand…"): what people actually DO is the report\'s to tell.',
        ...
        'ALWAYS: sentences read once and understood — each parses ONE way, subject and verb early; orient a person ONCE, plainly; race named at most once and only when it matters; never echo these instructions or field names in prose — "step", "plan", "approach", "focal", "beat" are system words that never appear on a card; period diction (no word this world\'s smiths, priests, and farmhands could not know). One prop is BANNED as a plot object (the trade\'s most overused): the account-book — ledger, manifest, registry, record-book by any name; any other period document serves.',
        TAGS_NOTE,
        NUMBER_BAN,
        input.kind !== 'one-off' ? '═══ SAGA CARD ═══\nThis card is ONE STEP of an ongoing story. Same briefing voice ("you"); open on the ongoing matter and what has just changed — no region context. Where this block and the rules above disagree, THIS BLOCK WINS; quarryTags is never output. fixNotes (when given) = defects a zero-context reader found in your REJECTED previous draft: write a fresh card with none of them.\nTHE RECORD (hard constraints): storyState.history = the saga\'s prior reports, oldest first — the settled truth. An object rests where the last report left it; people and places keep their exact names; finished work is never re-posed (a thing broken stays broken, a thing delivered stays delivered). Where the bible disagrees with the record, the RECORD wins. A holder or place change since the last report is told AS a change. lastBeatOutcome = what the previous step changed: open on the situation it created; between steps the world moves ONLY as it says (a step reported UNTAKEN is re-posed afresh — the world did not move). knownToPlayer entries are immovable; introducedNames = people already met (bare name; orient everyone else once). storyState.actorStates = WHEREABOUTS, the single authoritative truth of where each person and object rests: the card may not place them anywhere else, may not re-take what it says the company holds, and moves things only on-screen.\nTHE STEP: arcStep = the ONE step this card covers — the job is THIS step, nothing more; when the record shows its work already done or its target gone, derive the job from what genuinely remains instead (the loose end, the handover, the settlement).\nTHE DIET: the player learns the hidden truth one card at a time. Stage ONLY this step\'s ground; the cast, places, terms, and dangers of LATER steps stay off the card entirely — not as warnings, not in pay clauses (sole exception: the goal). Beyond introducedNames and the goal/arcStep\'s own names, introduce at most TWO people and ONE place by name; everyone else stays nameless by trade. FIRST USE of any person, place, or thing says plainly who or what it is — never "the X" for a thing the player has not met; what only the bible knows, the player has NOT met. When the dealt step is to FIND or LEARN something, the card poses the QUESTION only — the answer, though the plan may hold it, stays off the card (finding it is the job).\nTHE BIBLE (hidden — the player never sees it): situation = the full truth, a well this card draws ONE piece from, never a summary to relay. goal = the engagement — known from beat 1: state it in ONE plain sentence; only the finale settles it. cast = its people; an entry with offstage: true has NO name — when this step runs into its pressure, it shows nameless by trade in your own words. tensions/openDirections = pressure notes: ONE that this step\'s ground actually meets may color the card; resolve none. twist = a hidden recontextualizing fact. title/loreId/kernel = bookkeeping, ignore.\nNAMES & PLACES: name people ONLY from bible.cast, rosterNames, or relevantLore. Soldiers stay out of card prose (sole exception: the focal when focalIsMerc) and are never PINNED to a place or errand — where each soldier goes is the player\'s assignment alone. The bible\'s geography outranks placeNameSuggestions; a place never wears a person\'s name; the bible\'s coinages are notes — render them in your own plain words, grounded at first touch, never carried verbatim card to card.\nrelevantLore = what the world remembers: stay consistent with each tie and blurb. Flags: companySoldier = the player\'s own; companyCaptive = in the cells, and NO ONE ELSE is in custody; atTheFort = at the fort right now, never staged elsewhere; outOfReach = free in the world, never at the fort or in custody. focalDossier outranks the bible for the focal; the focal\'s SEX comes from their tags — never flip a pronoun mid-saga; a focal who is a soldier lives AT THE FORT, goes only where the player sends them, never hires or pays the company (the matter is company business around its own; pay comes from elsewhere or nowhere), and is never promised as a payment, prize, or recruit — they are already the company\'s.' : '',
        input.kind === 'beat' ? 'THIS IS A MID-SAGA STEP: never the saga\'s goal, and its objective must differ materially from the previous job (new ground, new claimant, new leverage, or raised stakes). The job must be WINNABLE: its success is a change the party can force — never another person\'s free choice, and never an outcome the card itself already forecloses. This step\'s obstacle is a NEW CLASS of trouble: never the previous step\'s obstacle re-armed, re-posted, or wearing a new name. A pressure an earlier card announced either ACTS on this card or is dropped — never re-announced. Delivery to the CLIENT is always later work: a mid-step may find, secure, or open the way, never end with the prize delivered. A twist storyState lacks surfaces at a MIDDLE step whose arcStep touches it. rewardEnvelope = side loot (the true prize lands at the finale; the client\'s promised pay covers profit); when this step\'s objective is a PERSON, the card may promise that person-outcome.\nBEAT 1 (beat field = 1): the player\'s FIRST sight of the saga — a cold reader gets the hook, the engagement, and this step\'s errand, NOTHING of the road beyond. It is taking up the matter plus the FIRST leg of field work (a fort-side meeting alone is not a job; when the dealt step holds none, the smallest first leg toward the next ground); keep it low-stakes. It alone carries the CARE MOMENT: one small human moment with the focal, aimed by the bible\'s telling — warmth where wronged, wariness where dangerous, never a lovable true villain; a focal who cannot be at the fort arrives in a witness\'s detail. Every LATER card carries no character business: people appear only in what the job needs. Each ask entry MAY add mustBeFocal: true — ONLY when focalIsMerc is true and this step stages that soldier\'s own matter in person; in doubt, omit.' : '',
        input.kind === 'finale' ? 'THIS IS THE FINALE — the arc\'s LAST step, bringing the matter to a head. It opens from the RECORD as it stands: what earlier steps did STAYS done; when the record shows the saga\'s work already complete, the finale stages the SETTLEMENT itself — the handover, the payment, the reckoning with whoever remains — never a re-run of finished work. It stands on ground and people the player has SEEN, puts knownToPlayer facts to USE (proofs, leverage, allies shape the plans), and contradicts none. It never reframes the focal against the saga\'s telling: a protected person stays protected, an accused stays merely accused, unless a REVEALED twist says otherwise. When the bible still holds a twist the player never met, the finale REVEALS it as part of the matter coming to a head — the truth comes out at the end, never quietly dies.\nPLANS: slotCount counts the mutually exclusive PLANS — the company sends soldiers down ONE plan, never a party of slotCount. Output approaches with EXACTLY slotCount entries and ask with the SAME count in the SAME order: ask[i] is plan i\'s test — same attribute, favored echoing the plan\'s; no clashing or requiredTag on plans. ALL plans settle the same central person (focalName), each testing a different attribute. rewardKind = what the COMPANY nets: recruit = they join the company; captive = the company holds them; gold = they pass out of the company\'s hands for value (ransomed home, paid off, claimed by kin). Offer only kinds the saga\'s telling can honor — a protected focal is not put in chains unless a revealed twist licenses it; when focalIsMerc is true no plan trades the soldier away or targets them with force, binding, or custody: each settles the MATTER and the soldier stays the company\'s own. A label promises ONLY what its rewardKind delivers — captive reads seize/hold, gold reads sell/collect the price, recruit reads win them over — never a fate (freedom, release, mercy) no kind grants, never a handover on a captive plan, never a prize object the envelope keeps changing hands (word such plans around the claim, the price, or the right). Labels are field orders in the world\'s plain words, name only people the situation itself grounds, and no plan may presuppose a twist the player never met. The situation presents the matter at its head and may sketch the choice in one sentence; the labels carry the fork. rewardEnvelope names the saga\'s central prize and the DEFAULT ending — the plans decide what actually lands, and the standalone envelope cap does not bind them.' : '',
      ].filter(Boolean).join('\n');
    ...
    async genesis(input: GenesisInput): Promise<GenesisOut> {
      ...
      const system = [
        ...
        '- focal: the person this saga is ABOUT — core cast; their tags are central to what it is about; their dossier (when fuller) is what the world remembers of them. isExistingMerc true = they already serve the player. kind = how the saga likely ENDS — recruit: the focal may end up joining the company (their role is usually prize); captive: may end in its cells (role: quarry); gold-hoard: the prize is a treasure the focal is the key to (role: quarry); development: a saga about one of the company\'s own, grown from ONE hook already in their dossier or backstory (their past stirring, not a fresh ambition) — the soldier LIVES AT THE FORT and marches with the company: the matter comes TO them (a petitioner, a summons, the past resurfacing at the gate), never the soldier staged dwelling, waiting, or ailing somewhere else; it ends with the matter around the soldier settled and the soldier still the company\'s own (their role: companion). Aim the arc at that ending.',
        '- slate: people this world already knows (relationPhrase = the tie; blurb = their tag-line). Reuse slate people before coining new ones — but a reused person keeps the SIDE their blurb and other sagas give them (never flipping ally to villain unexplained) and never serves as the client of two sagas at once; when their history with the company is dark — they fled it, robbed it, fought it — the situation carries that standing plainly (the company remembers); blurbs and dossier lines are SETTLED history — build on them, never overwrite. EVERY cast entry for an existing person — slate or focal — carries that person\'s id as loreId; omit loreId only for newly coined people. Flags: companySoldier = the player\'s own soldiers — CONTEXT, never cast entries (sole exception: a saga ABOUT one of them), and never clients, claimants, victims, or antagonists (the company does not hire, pay, or petition itself); companyCaptive = in the company\'s cells — and NO ONE ELSE is in the company\'s custody: never build on someone being held at the fort unless that flag says so; atTheFort = physically AT the fort right now — never cast leading, dwelling, or acting elsewhere; outOfReach = passed out of the company\'s reach — free in the world, never at the fort or in its custody.',
        '- avoid: the player\'s recent sagas — do not reuse their premises, central objects, central PLACES, rites/devices, title shapes, or DISPUTE SHAPE (how the matter is contested and settled must differ visibly from every entry); set this saga apart from all of them. A [SETTLED: …] tail is finished history: never re-stage its events as upcoming, and never contradict how it ended.',
    ...
    async resolve(inputs: ResolveQuestInput[]): Promise<ResolveQuestOut[]> {
      ...
      const system = [
        ...
        'OUTCOME MEANINGS: success = the job done clean — the JOB AS WRITTEN, no more and no less: EVERY thing the job line asked for lands (a success may not quietly drop one of them), and never take, deliver, or finish what the job only asked to find, learn, or scout. partial = done, but at a COST you must SHOW (a wound, a complication, a lesser haul). failure = the job NOT done; a consequence lands.',
        'BE CLEAR ABOUT THE RESULT: the reader must finish knowing EXACTLY what was achieved or lost and what the company now holds or knows. When the job\'s verb is to learn or uncover, the result IS the answer found (or not found) — state it in full: a report that says a name or fact was learned without SAYING it has reported nothing. When the answer is a person this message never names, name them by role or trade — never coin a name. An absent client is not staged as a character — say plainly how the promise stands. A promised UNCANNY thing shows itself behaving as promised in ONE concrete moment, never handled as ordinary goods.',
        'fixNotes (when given) = defects a zero-context reader found in your REJECTED previous report: write the report afresh with none of them.\n═══ SAGA STEP (when chainContext is given; without it the job stands alone) ═══\nThis job is ONE STEP of a longer saga. THIS STEP ONLY: the report performs chainContext.arcStep and nothing else. stepsNotYet = the plan\'s later steps — their work, prizes, and targets may not land, advance, or resolve here, however big the roll (a big roll is THIS step done exceptionally well). Unless isFinale is true, the saga\'s goal stays unachieved whatever the dice said; when a success as written would settle it, complete the JOB while the larger matter visibly stays open. What a resolution settles STAYS settled; storyState is the PAST — an event it records is done, never re-staged, and this report never repeats a prior report\'s event sequence: a second attempt at the same task goes DIFFERENTLY (new means, new turn, new cost). storyState.actorStates = WHEREABOUTS, the single truth of where each person and object rests: the report starts them THERE, and only what happens on screen may move them. A focal who is a company soldier is never handed into anyone\'s custody mid-saga. bible = the hidden truth (situation, goal, cast, twist; arc/tensions = the plan — the STATE outranks it; loreId/actorStates = bookkeeping).',
        'Produce, in order:',
        ...
        '2) "after": what happened, knowing the outcome. NEVER open by restating the job or its wording — the first sentence is the decisive moment or its immediate result. Report events in the order they mattered: how the attempt met the challenge, what it cost, what the company now holds or knows. Name a party member only where they personally turned the job — won it, botched it, found the thing, took the hurt; a member with no such moment gets NO invented one, and never describe a member with their trait word or its adverb. ONLY the soldiers of THIS party appear in this report — every other company soldier is at the fort and unnamed here. Between jobs every soldier returns to the fort: no report leaves a soldier posted, waiting, or holding something in the field — what the party took passes to the company, not to a soldier\'s arms. WEAVE items and people from deliveredSummary into the action as things changing hands in-fiction — never repeating its amounts or wording, nor "(npc)" or any parenthetical (gold there is the engine\'s tally; nothing to weave when it lists only gold). GOLD IS NEVER STAGED: no purses, coin-counting, or payment moments — the engine reports pay; the one exception is a job whose story IS the payment. deliveredSummary is what ends in the COMPANY\'s hands; if the job\'s wording seems to promise away something it says the company KEEPS, the company\'s take wins — the fiction explains how. End where the story stops — the deed done or what it sets in motion, always a concrete event or holding, never an abstract summing-up sentence; walking back to the fort is the overused closer. EVERY sentence parses ONE way on one skim: subject and verb arrive early, and a list of things brought back holds only what hands can carry. The after never NEGATES what the before staged — a threat the before put on screen acts or is dealt with, never reported absent — and a non-event is reported only when the job was to prevent it.',
        'CONTINUITY: a party member\'s dossier is what the world remembers of them. Let a memory or habit surface ONLY when this scene genuinely calls it up — one touch per person at most, MOST resolutions need none, never narrated as habitual ("as she always did" is a stamp), and a surfacing habit is expressed through a NEW action, never the dossier\'s gesture verbatim. A callback may reference ONLY events written in the dossier or storyState — an invented memory reads true once and false forever. A person who appears only in a dossier memory may be remembered, never staged as present. A name the job text and storyState never carried does not debut here as if known: such a person enters with one plain orienting word (their trade or side) or stays nameless. Every person keeps the sex their first mention gave them.',
        ...
        '═══ FINALE (when chainContext.isFinale) ═══\nThe ENGINE decides every disposition — who joins, leaves, dies, owns: narrate what was delivered as given, never declaring those yourself. chainContext.fate = what becomes of the central person; it COMPOSES with the outcome: the job\'s own objective resolves on screen FIRST, the fate lands after, never instead. End the person exactly on the fate in the fiction\'s own words: kept WITH the company = the report ends with them back with the company (an own soldier simply remains a soldier, never in custody); sent OUT of the company\'s hands = they leave into the arrangement (sold, ransomed, handed over), never escaping or kept after all. chainContext.approach = the plan the company CHOSE, a CONTRACT: the first after-sentence shows it executed by its own terms, every action its label names happens or fails on screen, and a failure fails THE CHOSEN plan; what made each rejectedApproaches plan DIFFERENT — its distinctive route, trick, or prop — may not appear (an action the job itself requires stays free). The report also ACCOUNTS for every named captive, prize, and open obligation still live in storyState — kept, returned, lost, or written off, each in a clause.',
        ...
        'Never echo these instructions or field names in prose — "approach", "plan", "roster", "lead", "envelope", "outcome", "step", "chosen" are system words: show the thing itself, never name the machinery. Period diction throughout: no word for a thing this world\'s smiths, priests, and farmhands could not know.',
        ...
        'storyUpdate: ONLY when chainContext is given; omit otherwise. Its truth SCALES with the outcome: success = the full new fact learned; partial = part of it, hedged or bought dear; failure = nothing concrete. currentSituation states CONCRETELY what changed — who holds what, who moved where — keeping every person and place name spelled EXACTLY as earlier text spelled it. actorUpdates = the WHEREABOUTS ledger: one entry per person and OBJECT this report moved or changed — {"name": "where they now are / who now holds it"} — only those that changed; the game carries it forward as the single truth of where everything rests. newlyRevealed holds only facts NOT already in the storyState.',
        TAGS_NOTE,
        'Respond as JSON matching: {questId, before, after, injuries:[{characterId, band: STRICTLY "low"|"med"|"high", cause: a phrase copied from your after text showing the harm — only the harmed appear here at all}], fleshed:[{characterId,who,backstory,quirks}], edges:[{from,to,type,blurb,importance}], storyUpdate?:{currentSituation, newlyRevealed: [plain strings], openThreads: [plain strings — the saga\'s live loose ends after this job, replacing the old list], actorUpdates: {name: "one line — where that person or object now rests / who holds it", only entries this report changed}, sagaSettled: true ONLY if this outcome left the saga\'s central matter essentially settled with nothing real left to do — a step that delivered the saga\'s prize or removed its central threat IS settled — the game will then bring the saga to its head next step (false on an ordinary step; on a finale there IS no next step — it is simply whether the matter ended settled)}}',
      ].join('\n');
