  ...
  return {
    ...
    async writeQuest(input: QuestWriteInput): Promise<QuestWriteOut> {
      ...
      const system = [
        ...
        '- situation: THE card the player reads. A common everyday job = 3-5 short sentences, never more; bigger matters up to three short paragraphs. Shape: how word reached you → the matter and who wants it done → the task, the hands it wants, the pay and risk. The task must be unmistakable from this text alone. A client\'s exact words may be quoted when they matter — never as a ritual.',
