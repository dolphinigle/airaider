...
function oneOffSystem(input: QuestWriteInput): string {
  ...
  return [
    ...
    '═══ YOUR INPUTS ═══',
    '- location: the land and its anchor facts. A named landmark may be used bare (never with an epithet). Any OTHER place is placeNameSuggestions[0], pasted whole and exactly as given — it arrives already introduced, so it is never trimmed to the bare name. You coin no place of your own, and no card names more than one.',
    ...
    archetypeLine(input.archetype),
    input.framedCharacter ? '- framedCharacter: the person the job delivers — the ONE person on this card who carries a given name. They arrive in the situation as WHAT THEY ARE, with the name following in that same sentence after a comma (\'a river pilot, Bokeeus Tidecaller\'), and no other field uses the name before the situation has; match them exactly (name, pronoun, tags). A dossier or lastSeen means the world knows them: continue their story in NEW words (lastSeen\'s FACTS are settled — captors, place, cause may not change).' : '',
