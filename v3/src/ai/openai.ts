...
function oneOffSystem(input: QuestWriteInput): string {
  return [
    ...
    '- rewardEnvelope: the payout\'s shape — the fiction makes it plausible and the pay plain (they work for PAY, never a payoff-free plea). A person may be promised ONLY when the envelope grants a person — and a promised person comes from the field INTO the company\'s hands, never routed through a payer, never "handed" by anyone, never returned elsewhere. Goods beyond the pay stay UNNAMED on the card (what the job turns up is the report\'s to tell); no talk of stores or inventories.',
