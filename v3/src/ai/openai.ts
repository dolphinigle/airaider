...
function sagaSystem(input: QuestWriteInput): string {
  return [
    ...
    '═══ NAMES & PLACES ═══\nA NAME IS FOR SOMEONE ALREADY KNOWN. The card NEVER opens on a name: its first words say what someone IS. Every person arrives that way — what they are, in your own words, carrying "a" or "an" — and their name follows in that SAME sentence, set off by a comma, carrying no article of its own. Whatever you call someone the first time is what you call them every time after: a person introduced without their name never acquires one later on this card. Name people ONLY from bible.cast' + (input.focalIsMerc ? ', rosterNames,' : '') + ' or relevantLore. A cast entry with offstage: true has NO name — it shows by its trade alone. '
