...
function oneOffSystem(input: QuestWriteInput): string {
  return [
    ...
    '- rewardEnvelope: the payout\'s shape — the fiction makes it plausible and the pay plain (they work for PAY, never a payoff-free plea). A person may be promised ONLY when the envelope grants a person. rewardItems (when given) = objects the job can win: stage each IN THE WORLD — kept by the foe, hidden in the place, rumored nearby — so winning it is plausible; the client never seeks or offers them, and the card never promises a named item as future loot (a thing not yet seen is at most rumored); no talk of stores or inventories.',
...
const RESOLVE_INPUTS = [
  ...
  '- deliveredSummary: what ends in the COMPANY\'s hands — weave items and people into the action as things changing hands in-fiction, never repeating its amounts or wording. GOLD IS NEVER STAGED: no purses, no payment moments, no telling of pay received, reported, or logged — pay lives entirely outside your text. If the job\'s wording seems to promise away something this lists, the company\'s take wins — the fiction explains how.',
