...
const desemi = (s: string) => {
  ...
  return s
    ...
    .replace(/(^|[.!?]\s+)(?:Expect|Count on|There will be|Likely)\s[^.!?]*[.!?]\s*/g,
      (m, p: string) => (ei++ % 2 === 0 ? p : m))   // drop every other forecast sentence whole
  ...
  return {
    ...
    async writeQuest(input: QuestWriteInput): Promise<QuestWriteOut> {
      ...
      const system = [
        ...
        '- rarity: sets SIZE and length — common = local trouble told short whatever its tone; uncommon and rare may run bigger. slotCount: how many soldiers the job takes.',
    ...
    async genesis(input: GenesisInput): Promise<GenesisOut> {
      ...
      const system = [
        ...
        '- focal: the person this saga is ABOUT — they must be core cast, and their tags are central to what it is about. Their dossier (when fuller than the tags) is what the world remembers of them. isExistingMerc true = they already serve the player. kind (top-level) = how the saga likely ENDS: recruit = the focal may end up joining the company (their cast.role is usually prize — they ARE what may be won); captive = the focal may end up in its cells (role: quarry); gold-hoard = the prize is a treasure the focal is the key to (role: quarry); development = a saga about one of the company\'s own. Aim the arc at that ending.',
    ...
    async resolve(inputs: ResolveQuestInput[]): Promise<ResolveQuestOut[]> {
      ...
      const system = [
        ...
        'THE PAIR: when two or more party members are present, include exactly ONE small exchange BETWEEN them — an act or a line. Their bond (or friction) is the long game; build it a brick at a time.',
    ...
    async flesh(inputs: FleshInput[]): Promise<FleshOut[]> {
      ...
      const system = [
        ...
        '- who: their CHARACTER-CARD line — the sentence under a hero\'s portrait. Shape: their station or origin, then ONE hook (a drive, a past, or a temper): "A [what they are/were]. [What drives or marks them.]" Two short plain sentences at most, third person. Identity-altitude only — never a micro-habit (habits live in quirks), never a metaphor or simile ("like a…" and "wore X like Y" are the tell), never merely their name, never a riddle or a poem.',
