...
function sagaSystem(input: QuestWriteInput): string {
  return [
    ...
    CARD_VARIANT === 'dlg'
      ...
      : '- situation: THE card. 3-5 short sentences (uncommon and rare may run longer). Shape: what has just changed, SHOWN in a dozen words or fewer as a thing anyone there could point at — never announced → this step\'s task as the outcome wanted → pay in its OWN short sentence, named in the world\'s words (the client\'s coin, goods off the dead), never echoing an instruction or field name. ONE FACT PER SENTENCE. The card TELLS the boss where the matter stands; it never orders them about — the task is named as the outcome wanted, never as an instruction to the reader. Name a thing by what it IS and what is wrong with it before any word about what it is made of or looks like. State intent and rumor, never a named person\'s scripted future action; risk lines are flowing prose, never a labeled clause.',
