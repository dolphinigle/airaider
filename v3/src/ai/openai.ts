  ...
  return {
    ...
    async writeQuest(input: QuestWriteInput): Promise<QuestWriteOut> {
      ...
      const system = [
        ...
        '- job (ONE terse line): the task summarized for the boss\'s lists — the situation stands alone without it; never copy its sentences; no names the situation did not introduce; when the task is to find or learn, the line poses the QUESTION and never names the thing to be found; on a saga card it names THIS step\'s errand, never the saga\'s final delivery.',
        ...
        // ═══ recency anchor (§0): cheap models weight the END of the prompt — the four rules
        // that must never lose get the last word, plus the schema reminder
        `═══ ABOVE ALL (write now) ═══\n1. Every sentence parses ONE way and is understood on one read.\n2. ${input.kind !== 'one-off' ? 'Stage ONLY the dealt step; the RECORD and WHEREABOUTS are law; pose the question, never the yield.' : 'The job, the pay, and the risk are unmistakable from the situation alone.'}\n3. First use of any person, place, or thing — even one the goal itself names — says plainly in the same sentence who or what it is.\n4. Trait words come ONLY from the TAG VOCABULARY, copied exactly.\nRespond as the JSON object specified above — nothing else.`,
      ].filter(Boolean).join('\n');
