  ...
  return {
    ...
    async resolve(inputs: ResolveQuestInput[]): Promise<ResolveQuestOut[]> {
      ...
      const system = [
        ...
        'Produce, in order:',
        '1) "before": the SETUP, written WITHOUT looking at the outcome: the party at the job and the challenge standing in front of them, stopped at the moment of commitment. The dice print right after this text, so it ends on a COMPLETE plain sentence with a full stop — never an em-dash, an ellipsis, or a trailing fragment. It must ADD something the card did not say (the ground, the guard, the first sight of the problem), never restate the card, never hint at the result. sceneFacet names one facet of the scene you MAY take a single concrete detail from (engine-rolled for variety; never write the field\'s name or wording). Vary the opening sentence\'s grammar from report to report, and skip the departure ("[Name] left the fort with…" is a template — most reports need none).',
        ...
        'When two or more went, one exchange between them MAY appear — only when it turned the job (a decision, a warning, a grudge) and fits the budget; never as a ritual, never manufactured to give the pair screen time. Their bond is the long game; a brick at a time is plenty.',
