...
function oneOffSystem(input: QuestWriteInput): string {
  return [
    ...
    CARD_VARIANT === 'dlg'
      ...
      : '- situation: THE card. Common = 3-5 short sentences; uncommon and rare may take up to three short paragraphs. Shape: the MATTER first (what is wrong and where) → who wants it done (one clause at most; none when the matter is visible from the walls) → the task as the outcome wanted (one errand, never an itinerary) → the pay LAST, as one clause turning on the work being done. ONE FACT PER SENTENCE. The FIRST sentence names what the job is about and the wrong in it — never a need, a wish, or what must happen — leading with whichever the reader needs first: the one wronged, the one who did it, or the word that reached the fort. Keep it to TWELVE WORDS OR FEWER, its subject and verb in the first four, and never as a command; where they went and who chased them wait for a later sentence. Someone may bring word, but an arrival that carries no fact of its own is cut. THE SENTENCE AFTER IT is the one that makes the matter bite, and it may run longer than the rest: it names the person whose act caused this, what they are to the others, and that act itself. NAME A THING BY WHAT IT IS AND WHAT IS WRONG WITH IT before any word about what it is made of or looks like — a detail the reader cannot use yet is cut. The card knows only what its sources could know: a hidden thing is suspected or rumored, never stated as fact — finding out is the job. Most cards need no risk line; when one appears, flowing prose, never a labeled clause. State intent and rumor, never a named person\'s scripted future action.',
...
function sagaSystem(input: QuestWriteInput): string {
  return [
    ...
    CARD_VARIANT === 'dlg'
      ...
      : '- situation: THE card. 3-5 short sentences (uncommon and rare may run longer). Shape: what has just changed, SHOWN in a dozen words or fewer as a thing anyone there could point at — never announced → this step\'s task as the outcome wanted → pay in its OWN short sentence, named in the world\'s words (the client\'s coin, goods off the dead), never echoing an instruction or field name. ONE FACT PER SENTENCE. Never DEFINE a person — no sentence whose whole work is saying who someone is or what they generally do; each person appears in the act that concerns this step. The card TELLS the boss where the matter stands; it never orders them about — the task is named as the outcome wanted, never as an instruction to the reader. Name a thing by what it IS and what is wrong with it before any word about what it is made of or looks like. State intent and rumor, never a named person\'s scripted future action; risk lines are flowing prose, never a labeled clause.',
