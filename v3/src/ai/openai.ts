...
function oneOffLightSystem(input: QuestWriteInput): string {
  return [
    ...
    (input.scouting && process.env.SCOUT !== '0'
      ? '- situation: ONE SENTENCE: a place where word gathers — a common room, a market, a crossing — who passes through it, and the kind of trouble they grumble about. NOBODY BROUGHT THIS IN and nobody is hiring you: the company goes to LISTEN and comes back with word of paying work. Nothing is wrong at the place itself, and no single incident is named.'
      : process.env.SEEN !== '0'
