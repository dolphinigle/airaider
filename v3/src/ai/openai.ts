...
function sagaSystem(input: QuestWriteInput): string {
  return [
    ...
    (input.beatIndex && input.beatIndex > 1) || input.kind === 'finale' || input.lastBeatOutcome
      ...
      : '',
    '═══ THE STEP ═══\narcStep = the ONE step this card covers — the job is THIS step, nothing more; when the record shows its work already done or its target gone, derive the job from what genuinely remains (the loose end, the handover, the settlement). '
    ...
    ASK_SPEC,
    '═══ ABOVE ALL (write now) ═══\n1. Every sentence parses ONE way and is understood on one skim — subject and verb early.\n2. Stage ONLY the dealt step: the cast, places, terms, and dangers of LATER steps stay off the card entirely' + (input.storyState ? '; the RECORD and WHEREABOUTS are law' : '') + '.\n3. First use of any person, place, or thing — the client included — is introduced by a FULL sentence with a verb of its own that grounds who or what they are in this matter; a card whose reader cannot say who hires them, what the matter is, and why it matters has failed.\n4. Every word in favored, clashing, or requiredTag is copied EXACTLY from TAG VOCABULARY; period diction; never echo an instruction or field name ("step", "plan", "focal", "beat", "arc", "the hire" never appear on a card); the account-book is BANNED as a plot object.\nRespond as the JSON object specified above — nothing else.',
  ].filter(Boolean).join('\n');
}
