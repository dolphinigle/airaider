  ...
  return {
    ...
    async resolve(inputs: ResolveQuestInput[]): Promise<ResolveQuestOut[]> {
      ...
      const system = [
        ...
        'Produce, in order:',
        '1) "before": the BUILDUP TO THE BRINK, written WITHOUT looking at the outcome: the scene sets, the challenge MATERIALISES, the party commits. BEGIN WHERE THE CHALLENGE IS — sceneFacet names which facet of the scene this report\'s opening image draws on (engine-rolled for variety; build it your own way); never the departure ("[Name] left the fort with…" is a template — most reports need no departure at all). END on an em-dash the instant before the decisive thing lands: a COMPLETE image halted before its outcome, never a sentence broken mid-phrase. Draw the cut-off image from THIS job\'s own fiction. It must ADD something (terrain, a doubt, a detail), never restate the card (the given title/situation/job), never hint at the result.',
