...
function sagaSystem(input: QuestWriteInput): string {
  return [
    ...
    '═══ NAMES & PLACES ═══\nA NAME IS FOR SOMEONE ALREADY KNOWN. Every person arrives first by WHAT THEY ARE — their trade or their who, in your own words with "a"/"an" — and their name, if used at all, follows inside that same sentence, never ahead of it. Name people ONLY from bible.cast' + (input.focalIsMerc ? ', rosterNames,' : '') + ' or relevantLore. A cast entry with offstage: true has NO name — it shows by its trade alone. '
    ...
    CARD_VARIANT === 'dlg'
      ? '- situation: THE card — this step\'s news arrives in a VOICE. First line: the teller alone in square brackets — whoever the record makes its natural bearer: the client by name once met, their messenger, or word sent back from that ground; on the saga\'s first card, the client themselves. Then the teller\'s OWN words, first person, in their station\'s diction (plain period words): what has just changed, SHOWN as a thing anyone there could point at rather than announced → this step\'s task as the outcome wanted → the pay in its OWN short sentence, named in the world\'s words (the client\'s coin, goods off the dead), never echoing an instruction or field name. ONE FACT PER SENTENCE. The teller says only what the record lets them know. State intent and rumor, never a named person\'s scripted future action.'
      : '- situation: THE card. 4-7 short sentences. Shape: the person and what they are after, SHOWN — never announced → why it matters to them → this step\'s task as the outcome wanted → what stands against it → pay in a CLAUSE, named in the world\'s words (the client\'s coin, goods off the dead), never echoing an instruction or field name. ONE FACT PER SENTENCE. The card TELLS the boss where the matter stands; it never orders them about — the task is named as the outcome wanted, never as an instruction to the reader. Name a thing by what it IS and what is wrong with it before any word about what it is made of or looks like. State intent and rumor, never a named person\'s scripted future action; risk lines are flowing prose, never a labeled clause.',
  ...
  return {
    ...
    async writeQuest(input: QuestWriteInput): Promise<QuestWriteOut> {
      ...
      const user = JSON.stringify({
        ...
        standsAgainst: input.knownObstacle, clientTell: input.tell, noClient: input.noClient || undefined,
