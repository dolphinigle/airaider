...
const NUMBER_BAN =
  ...
  'Character names must come from the names this message gives you — never coin your own. Keep prose tight; plain low register — no archaic diction, no modern idiom, and no object or term from after the age of candles and horses; concrete nouns. ' +
  ...
  return {
    ...
    async writeQuest(input: QuestWriteInput): Promise<QuestWriteOut> {
      ...
      const system = [
        ...
        '- gravity (when given): sets TONE only — a small everyday job reads brisk and workmanlike; a serious matter reads straight; only a grave affair reads heavy. rarity sets SIZE and length: common = local trouble, told short, whatever its tone. slotCount: how many soldiers the job takes.',
        ...
        '- framedCharacter (when given): the person the job delivers — the one person who MUST carry their given name; the card must match them exactly (name, pronoun, tags). A dossier or lastSeen means the world already knows them: continue their story — another try at a known matter, never fresh news, retold in NEW words (never copy lastSeen\'s wording). npcNameSuggestions (when given): names for at most one or two others, only if the job truly needs them named.',
        ...
        '- situation: THE card the player reads. A common everyday job = 3-5 short sentences, never more; bigger matters up to three short paragraphs. Shape: THE MATTER ITSELF FIRST — what is wrong and where — then who wants it done (how word reached you is worth one clause at most, and when the matter is visible from the walls or already known, no bringer is needed at all) → the task, the hands it wants, the pay and risk. Never open on the arrival of a messenger. The task must be unmistakable from this text alone. A client\'s exact words may be quoted when they matter — never as a ritual.',
    ...
    async resolve(inputs: ResolveQuestInput[]): Promise<ResolveQuestOut[]> {
      ...
      const system = [
        ...
        'Produce, in order:',
        '1) "before": the BUILDUP TO THE BRINK, written WITHOUT looking at the outcome: the scene sets, the challenge MATERIALISES, the party commits. BEGIN WHERE THE CHALLENGE IS — the place, the obstacle, or the person waiting; never the departure ("[Name] left the fort with…" is a template — most reports need no departure at all). END on an em-dash the instant before the decisive thing lands — the leap begun, the door giving way, the blade raised: a COMPLETE image halted before its outcome, never a sentence broken mid-phrase. Draw the cut-off image from THIS job\'s own fiction. When this message carries more than one quest, no two brinks open with the same sentence shape. It must ADD something (terrain, a doubt, a detail), never restate the card (the given title/situation/job), never hint at the result.',
        ...
        'Vary the lead-in clause — not every departure is mist, rain, fog, or a time of day ("Dawn found them…" is a stamp). Vary hurt too: not every wound is a gashed palm or forearm — legs, ribs, scalp, an eye swollen shut — and never the same wound SENTENCE twice ("took a cut to her [part] from a [thing]" is a stamp: vary the how and the words), and the wound rides INSIDE its action beat — never stapled onto the end of an unrelated sentence; never narrate a wound you did not list in injuries.',
        'On failure: state in-fiction what the party came home without — NEVER the canned words "the reward is lost" or "nothing —".',
        'WORD BUDGET by rarity: common → before 2 short sentences, after 2 sentences MAX. uncommon → 3/3. rare, or a saga finale (chainContext.isFinale true) → 4/5 (the payoff moment — keep the brink and resolution generous). The after budget grows by ONE sentence per party member beyond the first (each member\'s beat needs its own sentence under the one-action rule). Respect it strictly.',
    ...
    async flesh(inputs: FleshInput[]): Promise<FleshOut[]> {
      ...
      const system = [
        ...
        '- who: ONE line they would be known by around the fort — a plain FACT about them, specific and human, never a metaphor or simile ("like a…" and "wore X like Y" are the tell), never generic, never merely their name ("keeps the night watch nobody else wants" beats "a brave fighter"). Plain everyday words a player remembers; never a riddle or a poem.',
