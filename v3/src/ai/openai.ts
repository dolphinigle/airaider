  ...
  return {
    ...
    async genesis(input: GenesisInput): Promise<GenesisOut> {
      const system = [
        ...
        'assignedNames = the ONLY names for coined people, sex-marked: match both ways, order free, the marks never output. newPlaces = coined places, never sharing a first syllable with the landmark or any place this message names; each blurb one plain sentence under fifteen words. Titles are concrete ACTION-titles, never a poetic two-noun. ' + (process.env.LAWWORDS === '0' ? 'Law and custom speak in period words, never modern ones.' : 'Law and claims speak in period words — rights, pledges, sworn witness.') + ' One prop is BANNED anywhere in the bible: the account-book — ledger, manifest, registry, record-book by any name. Never echo these instructions or field names in prose — "the hire" is instruction-speak: arc steps and situation call the client by NAME; no semicolons — split into two sentences.',
        NUMBER_BAN,
        EDGE_TYPES_LINE,
        TAGS_NOTE,
