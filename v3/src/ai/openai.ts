...
const desemi = (s: string) => s.replace(/;\s+(\S)/g, (_, c: string) => `. ${c.toUpperCase()}`);
...
const NUMBER_BAN =
  ...
  'Character names must come from the names this message gives you — never coin your own. Keep prose tight; plain low register — no archaic diction, no modern idiom; concrete nouns. ' +
  ...
  return {
    ...
    async resolve(inputs: ResolveQuestInput[]): Promise<ResolveQuestOut[]> {
      ...
      const system = [
        ...
        'Vary the lead-in clause — not every departure is mist, rain, fog, or a time of day ("Dawn found them…" is a stamp). Vary hurt too: not every wound is a gashed palm or forearm — legs, ribs, scalp, an eye swollen shut — and never the same wound SENTENCE twice ("took a cut to her [part] from a [thing]" is a stamp: vary the how and the words); never narrate a wound you did not list in injuries.',
    ...
    async flesh(inputs: FleshInput[]): Promise<FleshOut[]> {
      ...
      const system = [
        ...
        '- who: ONE line they would be known by around the fort — a plain FACT about them, specific and human, never a metaphor, never generic, never merely their name ("keeps the night watch nobody else wants" beats "a brave fighter"). Plain everyday words a player remembers; never a riddle or a poem.',
