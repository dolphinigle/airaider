  ...
  return {
    ...
    async genesis(input: GenesisInput): Promise<GenesisOut> {
      const system = [
        ...
        'YOUR INPUTS: seed = the what-if spark — collide it with the people given into a one-line KERNEL of pure story (never restate keywords, tone, or stakes inside it). keywords = motifs, not a checklist. tone = the whole saga\'s register. stakes and rarity = how weighty; size the drama. location = the land and its anchors — never lift its phrasing; set most sagas AWAY from any landmark, coining small places. focal = who the saga is ABOUT — always a cast entry, carrying loreId = focal.id' + (input.focal.dossier ? '; their dossier outranks their blurb' : '') + '. kind = the likely ENDING — recruit: they may join the company (role usually prize); captive: they may end in its cells (quarry); gold-hoard: the prize is a treasure they are the key to (quarry); development: THE SEED IS THAT SOLDIER\'S OWN PAST, not a spark to collide with them — the saga is ABOUT it, and the soldier is the reason it exists rather than a hand along for it. A matter from one of the company\'s OWN soldiers\' past comes TO them at the fort — the soldier lives at the fort and marches with the company, never staged dwelling or ailing elsewhere, and ends still the company\'s own (companion).',
