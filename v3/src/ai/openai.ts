  ...
  return {
    ...
    async writeQuest(input: QuestWriteInput): Promise<QuestWriteOut> {
      ...
      const system = [
        ...
        TAGS_NOTE,
        NUMBER_BAN,
        input.kind !== 'one-off' ? 'THIS CARD BELONGS TO AN ONGOING SAGA — same briefing voice ("you"); skip broad region context and open on the ongoing matter and what has just changed. Where this block and the standalone rules disagree, THIS BLOCK WINS; quarryTags is never output on a saga card.\nbible = the hidden settled truth — the player NEVER sees it: they learn it ONE CARD AT A TIME, like a serial, and this card may surface at most ONE new layer of it. THE CARD\'S DIET: stage ONLY the dealt arcStep\'s ground — where this step goes, who it deals with, what must be done there. Cast, places, terms, and dangers that belong to LATER arc steps stay OFF the card entirely — not as a warning, not in the pay clause: the player meets them when the saga arrives there (sole exception: the saga\'s goal, below). Past introducedNames — and past names the goal or the dealt arcStep themselves carry, which are already dealt and never count — a card may introduce at most TWO people by name and ONE named place; everyone and everywhere else stays nameless by trade or plain description. kernel = one-line idea; situation = the full hidden truth — a well this card draws ONE piece from, never a summary to relay; goal = what the company is engaged to do — the player knows it from beat 1: state it in ONE plain sentence, and only the finale may settle it; arc = the step guide, but when lastBeatOutcome or storyState contradict it the STATE wins: re-derive the objective from where things actually stand. What an earlier step delivered or settled is DEAD GROUND — never asked again in any costume, never resurrected (a thing broken stays broken). cast = its people (who / want / role) — an entry carrying offstage: true is NOT on this step\'s stage: it is given WITHOUT a name; when the dealt step itself runs into their pressure, it shows nameless by trade IN YOUR OWN WORDS, and you never coin a name for them. twist = a hidden recontextualizing fact (may be null); tensions and openDirections = pressure notes — let ONE color this card\'s complication, resolve none, and the DIET binds them too: only a pressure THIS step\'s ground actually runs into may show; title, loreId = bookkeeping, ignore.\nNAMES: people may be named ONLY from bible.cast, rosterNames, or relevantLore — no others; soldiers still stay out of card prose unless the focal IS one of them (focalIsMerc); a person named only inside bible.situation surfaces by name only when their own step puts them on stage. The bible\'s places and geography outrank placeNameSuggestions and the location field — stage the card where the saga says. The bible\'s coinages are NOTES, never fixed names: render its things and devices in your own concrete words, ground an invented conceit in one plain clause the first time a card touches it (a cold reader must parse beat 1 with no prior context), and never carry a bible phrase verbatim card to card.\nrelevantLore = what the world remembers: stay CONSISTENT with each entry\'s tie and blurb. Flags: companySoldier = the player\'s own; companyCaptive = in the company\'s cells — and NO ONE ELSE is in custody: custody exists only where that flag says so; atTheFort = physically at the fort right now, never staged acting elsewhere; outOfReach = free in the world, never at the fort or in custody unless won back on screen. focalDossier (when given) is fresher than the bible — any focal moment must fit it.\nstoryState = what the player has seen: currentSituation; knownToPlayer (a SETTLED: entry is immovable — build on it, never against it); openThreads; actorStates = bookkeeping, ignore; introducedNames = people already met (bare name; orient everyone else once). arcStep = the ONE step this card covers, dealt verbatim — this card\'s job is THIS step and nothing more. focalName = who the saga is about; focalIsMerc = whether they serve the company — such a focal is NEVER staged as the client hiring or paying the company (it does not hire itself): the matter is company business around its own soldier, pay comes from elsewhere or nowhere; the focal\'s SEX comes from their tags — check before writing a pronoun, never flip it mid-saga. lastBeatOutcome = what the previous step changed: open on the situation it CREATED, never re-pose the previous job (a step reported UNTAKEN is re-posed afresh — the world did not move); between steps the world moves ONLY as lastBeatOutcome says — never an invented offscreen capture, delivery, or rescue.' : '',
        input.kind === 'beat' ? 'THIS CARD IS ONE MID-SAGA STEP: it covers arcStep ONLY — never the saga\'s goal — and its objective must be materially different from the previous job (new ground, new claimant, new leverage, or raised stakes). Beat 1 (that is, when the beat field = 1) is the player\'s FIRST sight of the saga and must read clean to a cold reader — the hook, the engagement, this step\'s errand, and NOTHING of the road beyond; it = taking up the matter plus the FIRST leg of real field work — there must be a job to send soldiers on, but never the errand\'s heart; when the dealt step holds no field work of its own, the job is the smallest first leg toward the next ground. Delivery to the CLIENT is ALWAYS later work: a mid-saga job may find, secure, or open the way, never end with the prize delivered. When the bible holds a twist that storyState does not yet contain, a MIDDLE step whose arcStep touches it surfaces it as the card\'s one revealed layer. rewardEnvelope here is side loot — the saga\'s true prize lands at the finale, and the plain-profit rule is satisfied by the client\'s promised pay; when this step\'s objective is a PERSON (found, freed, escorted), the card may promise that person-outcome. BEAT 1 IS THE CARE BEAT: one small HUMAN moment with the focal, early but inside the normal card shape, its aim following the bible\'s own telling of them — warmth or pity where it paints them warm or wronged, wariness or dark curiosity where it paints them dangerous (never make a true villain lovable); if the focal cannot be at the fort, the moment arrives secondhand through a witness\'s detail. Keep beat 1 low-stakes. Each ask entry MAY add mustBeFocal: true — ONLY when focalIsMerc is true and the dealt step stages that soldier\'s own matter in person; when in doubt, omit the field entirely.' : '',
