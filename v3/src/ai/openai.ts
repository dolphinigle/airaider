...
function sagaSystem(input: QuestWriteInput): string {
  return [
    ...
    ASK_SPEC,
    '═══ ABOVE ALL (write now) ═══\n1. Every sentence parses ONE way and is understood on one skim — subject and verb early.\n2. Stage ONLY the dealt step: the cast, places, terms, and dangers of LATER steps stay off the card entirely; the RECORD and WHEREABOUTS are law.\n3. First use of any person, place, or thing — the client included — is introduced by a FULL sentence with a verb of its own that grounds who or what they are in this matter; a card whose reader cannot say who hires them, what the matter is, and why it matters has failed.\n4. Every word in favored, clashing, or requiredTag is copied EXACTLY from TAG VOCABULARY; period diction; never echo an instruction or field name ("step", "plan", "focal", "beat", "the hire" never appear on a card); the account-book is BANNED as a plot object.\nRespond as the JSON object specified above — nothing else.',
  ].filter(Boolean).join('\n');
}
  ...
  return {
    ...
    async genesis(input: GenesisInput): Promise<GenesisOut> {
      ...
      const system = [
        ...
        '═══ THE ARC — the story\'s spine ═══\nA SIMPLE, LINEAR story a tired player could retell in one breath, written as a CAUSAL CHAIN of EXACTLY expectedBeats steps. Each step: "<one errand at one place, using the previous step\'s yield> → yields: <the ONE thing found, learned, or changed that the next step uses>". Step 1 REACHES the first ground the hire named and turns up the first lead: its errand half names ONLY what the hire itself knows, and its yield POINTS onward (a fact, a sighting, an object that leads on), never the goal\'s own prize already taken, freed, or delivered — a fetch, rescue, or capture saga LOCATES or gains access at step 1, it does not grab. Every person and place the saga discovers enters as some step\'s yield, never before. Only the LAST step takes, frees, seizes, or delivers the goal\'s own prize; every earlier step changes the SITUATION — new ground, a barrier down, fresh leverage — and leaves the prize still to win, so a middle step that already grabs it is mis-scoped and guts the ending. The last step is also the arc\'s HARDEST: the CAST\'s own opposing entry stands in it and is beaten, bought, or outwitted THERE — never already overcome at an earlier step, never replaced by a new opponent invented for the ending; a final step of bare travel, pickup, or unopposed handover is mis-scoped. No two steps share a place or a person-outcome. Anyone the arc names must be in cast (or stay nameless by trade).',
