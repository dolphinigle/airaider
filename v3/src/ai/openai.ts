...
const EXPECT_SUBS = ['Count on ', 'Likely ', 'There will be '];
const desemi = (s: string) => {
  // start the substitute rotation from a TEXT hash — a per-call counter always started at 0,
  // so every card's one risk line became "Count on …" (×38/run: the new "Expect")
  let ei = [...s.slice(0, 40)].reduce((a, c) => a + c.charCodeAt(0), 0) % EXPECT_SUBS.length;
  return s
    .replace(/;\s+(\S)/g, (_, c: string) => `. ${c.toUpperCase()}`)
    .replace(/—\s*(and|but|then|so)\s*$/i, '—')   // dangling conjunction after a brink em-dash
    // STAMP-BREAKERS (mechanical — the named bans were ignored or, worse, echoed):
    // "Expect …" closed 20-27 cards/run; "the company's keeping" ×9-12 incl. broken grammar;
    // "Pay is coin." ×21/run
    .replace(/(^|[.!?]\s+)Expect\s+/g, (_, p: string) => p + EXPECT_SUBS[ei++ % EXPECT_SUBS.length]!)
  ...
  return {
    ...
    async resolve(inputs: ResolveQuestInput[]): Promise<ResolveQuestOut[]> {
      ...
      const system = [
        ...
        'Produce, in order:',
        '1) "before": the BUILDUP TO THE BRINK, written WITHOUT looking at the outcome: the scene sets, the challenge MATERIALISES, the party commits. BEGIN WHERE THE CHALLENGE IS — sceneFacet names which facet of the scene this report\'s opening image draws on (engine-rolled for variety; build it your own way and never write the field\'s name or wording); never the departure ("[Name] left the fort with…" is a template — most reports need no departure at all). END on an em-dash the instant before the decisive thing lands: a COMPLETE image halted before its outcome, never a sentence broken mid-phrase. Draw the cut-off image from THIS job\'s own fiction. It must ADD something (terrain, a doubt, a detail), never restate the card (the given title/situation/job), never hint at the result.',
