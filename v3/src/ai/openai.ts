  ...
  return {
    ...
    async writeQuest(input: QuestWriteInput): Promise<QuestWriteOut> {
      ...
      const system = [
        ...
        TAGS_NOTE,
        NUMBER_BAN,
        input.kind !== 'one-off' ? 'THIS CARD BELONGS TO AN ONGOING SAGA — same briefing voice ("you"), but skip broad region context: open on the ongoing matter and what has just changed. Where this saga block and the standalone rules above disagree, THIS BLOCK WINS. framedCharacter is a one-off field and absent here; quarryTags are never output on a saga card.\nSAGA INPUTS: bible = the story\'s hidden settled truth (the player NEVER sees it; this card may surface at most ONE new layer of it). Its fields: kernel = the saga\'s one-line idea; situation = the full hidden truth; goal = what the company is engaged to do — the ONE bible field the player already knows (not a secret; restating it costs nothing against the one-layer budget), and only the finale may settle it; arc = the rough step-by-step guide. Whenever lastBeatOutcome or storyState contradict the arc — whatever changed things — the STATE wins: re-derive the objective from where things actually stand, never from the plan. Anything an earlier step already delivered, handed over, or settled is DEAD GROUND: this card may not ask for it again in any costume, and may not resurrect a fact a report ended (a thing broken stays broken) — delivered means delivered, wherever the arc\'s wording points; cast = its people, each with who / want / role (their function in the story — e.g. client, companion, quarry, obstacle, ally, prize); twist = a hidden fact that recontextualizes the goal mid-saga (may be null); tensions and openDirections = pressure notes: let ONE color this card\'s complication, never resolve them; the bible\'s title and any loreId or actorStates entries are internal bookkeeping — ignore them.\nPeople in this card may be named ONLY from bible.cast, rosterNames (appearing solely as the company\'s own), or relevantLore entries — no other names; these pools OVERRIDE the standalone name fence — EXCEPT the roster: soldiers still stay out of card prose unless the focal IS one of them (focalIsMerc) — and a person named only inside bible.situation surfaces by name ONLY as this card\'s one revealed layer. Places the bible names outrank placeNameSuggestions, and the bible\'s own geography outranks the location field (which only names the wide region) — stage the card where the saga says it happens. The bible\'s own coinages are NOTES, never fixed names: render its things, creatures, and devices in your own concrete words — ground any invented conceit in one plain clause the first time a card touches it (a cold reader must parse beat 1 with no prior context), and never carry one bible phrase verbatim from card to card.\nrelevantLore (when given) = what the world already remembers around this saga: each entry is a person or place, with its tie (relationPhrase), its tag-line (blurb), and sometimes a fuller dossier of memories — the card must stay CONSISTENT with these. An entry flagged companySoldier is one of the player\'s own soldiers (the rosterNames rules apply); one flagged companyCaptive sits in the company\'s cells and cannot walk the world free — and NO ONE ELSE is in the company\'s custody: custody exists ONLY where that flag says so. One flagged atTheFort is physically AT the fort right now — never cast them leading, dwelling, or acting elsewhere. One flagged outOfReach has passed out of the company\'s reach — free in the world, never at the fort or in its custody unless a card wins them back on screen. focalDossier (when given) = what the world currently remembers of the focal, fresher than the bible — any focal moment must fit it.\nstoryState = what the player has seen so far: currentSituation = where things stand for them; knownToPlayer = facts they hold (an entry prefixed SETTLED: is engine-recorded and immovable — build on it, never against it); openThreads = loose ends; introducedNames = people already met (bare name only; orient everyone else once). beat / expectedBeats = which step this is of how many. arcStep = the ONE arc step this card covers, dealt verbatim: the card\'s job is THIS step and nothing more — never the goal, never a later step\'s work (the finale\'s arcStep is the arc\'s last). focalName = who the saga is about; focalIsMerc = whether they already serve the company; the focal\'s SEX comes from their tags in focalDossier and the bible — check before writing a pronoun, and never flip it mid-saga. lastBeatOutcome (when given) = what the previous step changed: open on the situation it CREATED and never re-pose the previous job (when it says the step went UNTAKEN, re-pose that step afresh — the world did not move). Between steps the world moves ONLY as lastBeatOutcome says — never invent an offscreen capture, delivery, or rescue to set this card up.' : '',
        input.kind === 'beat' ? 'THIS CARD IS ONE MID-SAGA STEP: it covers arcStep and ONLY arcStep (beat 1 = the taking-up of the matter and the first leg of its work — there must be a real job to send soldiers on — but never the errand\'s heart) — its job may NEVER be, or complete, the saga\'s goal, and its objective must be materially different from the previous job (new location, new claimant, new leverage, or raised stakes). The delivery to the CLIENT — the sought thing or person reaching the hands that engaged the company — is ALWAYS later work: a mid-saga job may find, secure, or open the way, never end with the prize delivered. When the bible holds a twistReveal that storyState does not yet contain, a MIDDLE step whose arcStep touches it is where it surfaces, as this card\'s one revealed layer. rewardEnvelope on a mid-saga step reads "side loot": minor incidental valuables a step MAY shake loose — the saga\'s true prize lands only at its finale, and the plain-profit rule is satisfied by pay PROMISED for the saga (a card is not payoff-free when the client\'s pay stands behind it) — and when this step\'s objective is a PERSON (found, freed, escorted), the card may promise that person-outcome: the goods-envelope governs incidental loot, never the saga\'s own objective. BEAT 1 IS THE CARE BEAT: before any plot pressure, one small HUMAN moment with the focal (it tells the player who this person is), its aim FOLLOWING THE BIBLE\'S OWN TELLING of the focal first, their cast role second: warmth or pity where the bible paints them warm, wronged, or merely the key to a prize; wariness or dark curiosity where it paints them dangerous (never make a true villain lovable at beat 1); the care moment rides INSIDE the normal card shape — matter first, the human moment early, not necessarily the opening sentence; if the focal cannot be present at the fort, that moment arrives secondhand through a witness\'s detail. Keep beat 1 low-stakes — its profit may rest on the client\'s PROMISE; nothing needs shaking loose yet. Each ask entry MAY gain one optional field, so its shape is {attribute, extraAttribute?, favored, clashing, requiredTag?, mustBeFocal?} — include mustBeFocal: true ONLY when focalIsMerc is true and this step is about that soldier\'s own past (it pins the slot to them); otherwise omit the field entirely.' : '',
        input.kind === 'finale' ? 'THIS IS THE FINALE — it covers the bible arc\'s LAST step and must bring the matter to a head. It opens from storyState AS IT STANDS: what earlier steps already did STAYS done — never re-staged as still to do; and when little has yet happened (a saga forced early to its head), the finale still stages ONLY the last step — the road to it is preparation folded into a clause, never itemized as separate tasks. It stands on ground and people the player has SEEN (storyState) — a bible place never yet shown may appear only introduced as new — and it puts to USE what storyState.knownToPlayer holds: hard-won proofs, leverage, and allies from earlier steps shape the plans, and the premise may not CONTRADICT a knownToPlayer fact — those are the player\'s own memories (a finale that forgets them re-fights won ground). And it never reframes the focal against the saga\'s own telling: a person the saga protected stays protected, an accused stays merely accused, unless a REVEALED twist says otherwise.\nPLANS: slotCount counts the mutually exclusive PLANS — the company sends soldiers down ONE plan, never a party of slotCount. Output approaches with EXACTLY slotCount entries, and ask with the SAME count in the SAME order: ask[i] is plan i\'s test (same attribute; its favored echoes the plan\'s). Each approaches entry is {label, rewardKind, attribute, favored: 1-3 vocabulary words} — no clashing or requiredTag on plans; approaches sits beside ask in the JSON object. ALL plans settle the same central person (focalName); rewardKind (recruit|captive|gold) names what the COMPANY nets: recruit = they join the company; captive = the company holds them; gold = they pass out of the company\'s hands for value (ransomed home, paid off, claimed by their kin). Offer only kinds this saga\'s telling can honor — a protected focal is not subdued into chains unless a REVEALED twist licenses it, and when focalIsMerc is true the plans NEVER trade the soldier away: each plan settles the MATTER around them and the soldier stays the company\'s own; each plan tests a different attribute; a plan\'s label may promise ONLY what its rewardKind delivers — captive labels read as seize/hold/keep, gold labels as hand over/sell/collect the price, recruit labels as win them to the company — never a fate (freedom, release, mercy) no rewardKind grants, and never a handover on a captive plan — and a label never promises away a PRIZE OBJECT the envelope keeps with the company: word such plans around the claim, the price, or the right, not the object changing hands. Labels are field orders in the world\'s plain words, never procedural jargon. The player picks ONE — the situation presents the matter at its head and may sketch the choice in one sentence; the plan labels themselves carry the fork. rewardEnvelope here names the saga\'s central prize and the engine\'s DEFAULT ending — the PLANS decide what the company actually nets, and the standalone envelope cap does not bind them.' : '',
      ].filter(Boolean).join('\n');
    ...
    async genesis(input: GenesisInput): Promise<GenesisOut> {
      ...
      const system = [
        ...
        '- focal: the person this saga is ABOUT — they must be core cast, and their tags are central to what it is about. Their dossier (when fuller than the tags) is what the world remembers of them. isExistingMerc true = they already serve the player. kind (top-level) = how the saga likely ENDS: recruit = the focal may end up joining the company (their cast.role is usually prize — they ARE what may be won); captive = the focal may end up in its cells (role: quarry); gold-hoard = the prize is a treasure the focal is the key to (role: quarry); development = a saga about one of the company\'s own — grow it from ONE hook already in their dossier or backstory (their past stirring, not a fresh ambition). Aim the arc at that ending.',
        '- slate: people this world already knows, each with how they connect (relationPhrase; their blurb is their tag-line). Reuse slate people before coining new ones (a slate with no usable people means coin freely) — but a reused person keeps the SIDE and standing their blurb and other sagas give them, never flipping ally to villain unexplained, and never serves as the client of two sagas at once. A slate person\'s blurb and dossier lines are SETTLED history: the bible may build on them, never overwrite them. EVERY cast entry for an existing person — slate or focal — carries that person\'s id as loreId (the link that ties them to the world); omit loreId only for newly coined people. companySoldier-flagged people are the player\'s own soldiers: they are CONTEXT, never cast entries (sole exception: a saga ABOUT one of them, whose role still comes from the enum). The situation may touch them only as the company\'s own — never as clients, claimants, victims, or antagonists (the company does not hire, pay, or petition itself). companyCaptive-flagged people sit in the company\'s cells and cannot walk the world free — and NO ONE ELSE is in the company\'s custody: never build the saga on someone being held at the fort unless that flag says so. An atTheFort-flagged person is physically AT the fort right now — never cast them leading, dwelling, or acting elsewhere. An outOfReach-flagged person has passed out of the company\'s reach — free in the world, never at the fort or in its custody.',
        ...
        TAGS_NOTE,
        'ARC SHAPE: step 1 = the company takes the job or meets the matter (the goal is NOT achieved here); middle steps = escalating turns that move across the world; the LAST step brings the matter to a head at the finale — never resolve the saga before it. No two steps may target the same delivery, the same place, or the same person-outcome — each step changes WHAT is at stake, not just where. Anyone the arc names must be in the cast (or stay nameless by trade in the step) — quest cards may only name cast members, so an arc built on an unnamed stranger strands its own steps. EXACTLY expectedBeats steps, each a short phrase.',
        ...
        'LAW AND CLAIMS speak in this world\'s words — right, claim, pledge, kin-law, witness, forfeit — never the vocabulary of modern courts and clerks.',
    ...
    async resolve(inputs: ResolveQuestInput[]): Promise<ResolveQuestOut[]> {
      ...
      const system = [
        ...
        'GAME WRITING, not literature: this is the field report the boss reads, with life in it — never a short story. Past tense, start to finish. Goods and people the company keeps come home to the company — never handed to an invented fort official (there is no quartermaster, sergeant, or clerk; the roster is the whole company) — and never spend a sentence saying the company keeps its own pay or prizes: the engine\'s own grant lines state that, so a custody sentence with nothing else in it is cut. Plain everyday English a tired player reads once and gets; short sentences, mostly one clause; no semicolons — split into two sentences instead. No similes anywhere in report prose ("like a…" is decoration — say the thing itself). In the AFTER text every sentence tells the boss something: what the soldiers did, what it cost, what they hold or know now — ONE action per sentence, under about twenty words, and never ACTIONS chained with "and" or commas ("X did A, did B, and did C" is three sentences — compound objects and plain descriptions are fine). In the BEFORE text every sentence must raise the stakes or show the challenge — terrain that matters, a doubt, the moment of commitment. Either way, a sentence that is only decoration is cut.',
        ...
        'BE CLEAR ABOUT THE RESULT: the reader must finish knowing EXACTLY what the company achieved or failed to achieve, and what they now hold or know — never vague, never mood-only. When the job\'s verb is to learn, uncover, or question, the result IS the answer found (or not found) — show what was learned, not merely an object carried home. When the answer is a person this message never names, name them by role or trade — that is a clear result, not vagueness; never coin a name for them. When the client is absent or unnamed, say plainly how the promise stands, without staging them as a character. When the job promised something UNCANNY, the report shows it behaving as promised in ONE concrete moment — never handled as ordinary goods.',
        'When chainContext is given, this job is one step of a longer saga. A focal who is one of the company\'s own soldiers is never handed into anyone\'s custody mid-saga — the matter around them travels; they stay with the company. chainContext.bible is the hidden truth behind it (its fields: kernel = the one-line idea; situation = the full hidden truth; goal = what the company was engaged to do; cast = its people with who/want/role — loreId and actorStates are bookkeeping, ignore them; twist = a recontextualizing fact or null; arc/tensions/openDirections = the writers\'-room plan, which the STATE outranks; chainContext.focalName names the saga\'s central person), chainContext.storyState is what the player has seen — and the storyState is the PAST: an event it records (a rite performed, a token cast, a prize taken) is DONE and may never be re-staged as if happening now; this job moves FORWARD from it. Unless chainContext.isFinale is true, this step may advance the saga but NEVER finish it: the bible\'s goal stays unachieved whatever the dice said — a success here succeeds at THIS job only, and the bible\'s arc lists steps that belong to LATER cards: this resolution may not perform, recover, or complete ANY of them (a beat that also does the finale\'s work makes every later card a rerun) — it performs chainContext.arcStep and ONLY that step — never a later step\'s delivery or errand, however well the dice rolled (finish the step short of the goal even when the fiction begs to go on). What a resolution SETTLES stays settled: a thing broken stays broken, an ended claim stays ended, a proof taken stays taken. When the dice say success but the job as written would settle the whole saga, the report completes the JOB while the larger matter visibly stays open, and storyUpdate must leave a live thread. Without chainContext the job stands alone.',
        'Produce, in order:',
        '1) "before": the BUILDUP TO THE BRINK, written WITHOUT looking at the outcome: the scene sets, the challenge MATERIALISES, the party commits. BEGIN WHERE THE CHALLENGE IS — sceneFacet names which facet of the scene the OPENING sentence draws on (engine-rolled for variety; one sentence, then the PARTY must appear — build it your own way and never write the field\'s name or wording), and vary the opening sentence\'s GRAMMAR from report to report — never the same sentence shape twice running; never the departure ("[Name] left the fort with…" is a template — most reports need no departure at all). The party MUST appear in the before-text — pure scenery may take at most its opening sentence; the rest shows the party moving, deciding, committing. END on an em-dash the instant before the decisive thing lands: a COMPLETE image halted before its outcome — someone or something in MOTION, never a static arrangement of objects, never a sentence broken mid-phrase. Draw the cut-off image from THIS job\'s own fiction. It must ADD something (terrain, a doubt, a detail), never restate the card (the given title/situation/job), never hint at the result.',
        'Party members are never the culprit, suspect, or wrongdoer of their own job — the company does not incriminate itself. The party list is COMPLETE: when one soldier was sent they are ALONE in the field — no "the others", no unnamed riders or party. A party member always comes home with the party — never left behind, lodged, or stationed elsewhere. After a person\'s first mention in a sentence, use their pronoun — never the same name twice in one sentence, and never "the elf/the man/the woman" as a name-substitute for someone already named. Each party member\'s pronoun comes from their tags — check before writing: a "female" tag is she/her in EVERY clause; one slipped "his" on a named woman breaks the character.',
        '2) "after": what happened, knowing the outcome. Give EVERY party member their own beat SHOWN through one concrete physical action — NEVER use their trait word or its adverb (no "playful", "instinctive", "calculating" in prose). WEAVE delivered ITEMS and PEOPLE from deliveredSummary into the action as things changing hands in-fiction (gold in deliveredSummary is the engine\'s tally — there is nothing to weave when it lists only gold). GOLD IS NEVER STAGED: no purses, pouches, coin-counting, or payment moments in prose — the engine reports pay separately; the one exception is a job whose story IS the payment. deliveredSummary is what ends in the COMPANY\'s hands once the job settles; anything the job promised to a client is separate — show it handled as the job said, and if the job\'s wording seems to promise away something deliveredSummary says the company KEEPS, the company\'s take wins — the fiction explains how. Never repeat deliveredSummary\'s amounts or its wording verbatim, nor "(npc)" or any parenthetical role. End the after-text where the story actually stops — on the deed done, on someone\'s reaction, on what it sets in motion; walking back to the fort or gate is the overused closer, never the default.',
        'CONTINUITY IS THE PRODUCT: a party member\'s dossier holds what the world remembers of them — possibly who they are known as, habits, and memories (each memory line names the other person, the tie between them, and what happened). When one is RELEVANT, let it surface. A person who appears only in a dossier memory may be REMEMBERED in prose but never staged as present. A callback may reference ONLY events written in the dossier or storyState — never invent shared history: an invented memory reads true once and false forever. Never info-dump a dossier; one touch per person at most, and MOST resolutions need none — a habit surfaces only when this scene genuinely calls it up, never as a signature stamped on every job — and never narrated AS habitual ("as she always did" is the stamp announcing itself). The dossier\'s identity line says WHO they are — it is not a prop to stage: a signature object from it may appear in at most the rare job where it matters.',
        ...
        'THE PAIR: when two or more party members are present, include exactly ONE small exchange BETWEEN them — an act as often as a line, and never the same clipped one-word whisper report after report. The exchange must CARRY something — a decision, a warning, a grudge, a comfort; bare touch with nothing passing between them is filler and is cut. Their bond (or friction) is the long game; build it a brick at a time.',
        ...
        'On failure: show in-fiction what the failure cost — the thing they meant to bring home and didn\'t, told fresh each time (never a fixed frame, never the canned words "the reward is lost" or "nothing —").',
        ...
        'Injuries: judge from the fiction per member — ONLY when the fiction itself put them in harm\'s way; a clean SUCCESS lists no injuries, and a failure that never put anyone in harm\'s way leaves NO wounds. Sometimes none even on violent failures; never death. List ONLY members who took harm; an empty array when nobody did. Each injury carries cause: the exact phrase FROM YOUR OWN "after" text that shows the harm happening, and it must NAME the harmed person taking their hurt inside their own action beat — a scene event alone ("the shaft caved") is not a wound; if your after text shows no such moment, there is no injury. Bands: low = walked off in days; med = weeks and a scar; high = months, nearly maimed — and the wound\'s LANGUAGE matches its band (a low wound reads as a nick or bruise, never a lodged spear).',
        'One prop is BANNED in prose (the trade\'s most overused): the account-book — ledger, manifest, registry, record-book by any name.',
        'The ENGINE decides and reports every disposition — who joins, leaves, dies, owns, and every promised lead\'s content; narrate only what was delivered as given, never declaring or hinting at any of those yourself (a promised trail is mentioned as promised, never invented in detail). On a saga finale, chainContext.fate states — as a plain sentence — what ACTUALLY becomes of the central person (focalName). The fate COMPOSES with the job\'s outcome, never replaces it: a won claim, rite, or rescue still succeeds AS WRITTEN, and the fate says where the person stands with the company when the dust settles — show both. chainContext.approach names the plan the company CHOSE — that plan is a CONTRACT: the after-text\'s FIRST sentence shows the chosen plan being executed by its own terms; every action its label names happens or fails ON SCREEN; and no action, prop, or verb belonging to a plan in chainContext.rejectedApproaches may appear (a failure shows THE CHOSEN plan failing — never a different plan tried instead). The chosen plan is HOW the job was attempted — the job\'s own requirements stay real alongside it. End the person exactly on the fate, told in the fiction\'s own words (never the fate\'s wording); when the fate keeps a person WITH the company but the plan handed them elsewhere, the report ends showing them back with the company — and one of the company\'s OWN soldiers is never worded into custody, holding, or the company\'s care: they simply remain a soldier of the company — and on a fate that sends them OUT of the company\'s hands, they leave into the ARRANGEMENT (sold, ransomed, handed over), never escaping or being kept after all. The fate is the last word. The company\'s-take-wins rule above governs GOODS; the finale person is governed by the fate alone.',
        'deliveredCharacters lists people the job handed over, each {id, name, tags}. Flesh each by the dossier rules: who = their CHARACTER-CARD line, shape "A [station or origin]. [One hook — a drive, a past, or a temper.]" — TIMELESS identity only: never current custody, quest-state, or willingness (those change; the line must not), never a micro-habit, metaphor, or simile, never merely their name; backstory = 2 sentences of concrete events growing out of THIS quest\'s fiction — how this person came to be in the mess the party found them in, carrying one detail a reader could love, pity, or worry over (a follower of the story must recognize them; spans and ages told in words are fine, prices and tallies are not); quirks = 1-2 concrete PHYSICAL habits, an action never an adjective (BANNED stock fidgets: fingering/thumbing an object, humming or whistling, wrist-rubbing, cloth-folding).',
