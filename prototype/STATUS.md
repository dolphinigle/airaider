...
> **Branch:** `prototype/m0`
> **Last verified command:** `npm test` → 228 / 228 passing
...
- [x] M13.1 Enemy-faction quest auto-stir (commit pending). `quests.json` schema gains optional `seededByEnemyFaction: string`; QuestSchema's `seededByTag` is now also optional. New finder `findEnemyFactionStirrableQuests(roster, catalog, enemyFactions)` returns quests whose enemy faction is currently at enemy tier and aren't already active/completed. `resolveDay` computes the end-of-day enemy-tier factions (post scenario rep deltas) and auto-stirs any matching quest, recording entries in `DayResolution.questsStirred` and appending a `[note]` fortLog entry. dayTranscript renders a `QUESTS STIRRED:` block. Seed catalog entry `lowmark-bounty` (2-stage bounty hunt) demonstrates the mechanic. 8 new tests + 6 day-mock goldens patched with `questsStirred: []`. (228 total)
